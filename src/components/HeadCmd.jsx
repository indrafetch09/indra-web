...
const HeadCmd = ({ pathname }) => {
  const command =
    routeCommands[pathname] ?? `show ${pathname.replace("/", "")} -c`;
  return (
    <>
      {/* head */}
      <div className="hidden sm:block">
        <div className="flex items-center gap-4">
          <span className="font-bold text-lg text-tosca">~</span>
          {/* horizontal line */}
          ...
          <div className="flex gap-2 text-sm font-bold text-main">
            ...
            <div className="flex gap-1 ubderline text-green">
              <FaGithub />
      ...
      <div className="flex text-sm gap-2">
        ...
        <span className="text-main">{command}</span>
      </div>
    </>
  );
};
